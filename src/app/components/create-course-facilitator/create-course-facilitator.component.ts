...
import {
  IcreateObjective,
  IcreateRequirement,
  IcreateMultiCourseTopicVid,
} from 'src/app/shared/Model';
declare var $: any;
declare var videojs: any;

@Component({
  selector: 'app-create-course-facilitator',
  templateUrl: './create-course-facilitator.component.html',
  styleUrls: ['./create-course-facilitator.component.css'],
})
export class CreateCourseFacilitatorComponent implements OnInit {
  ...
  objectiveString: IcreateObjective;
  requirementContents: any = [];
  requirementString: IcreateRequirement;
  courseTopicVideoContents: any = [];
  courseTopicVid: IcreateMultiCourseTopicVid;
  duration: any;
  fileSize: any;
  path: any;
  ...
  onSelect(event) {
    ...
    this.readFile(event.target.files[0]).then((fileContents) => {
      // Put this string in a request body to upload it to an API.
      console.log(fileContents);
      var courseImage = document.getElementById(
        'courseImage'
      ) as HTMLImageElement;
      courseImage.src = fileContents.toString();
    });
  ...
  createCourse() {
    ...
    const file_data = this.imageFile;
    ...
    this.authService.uploadImage(data).subscribe((response) => {
      if (response.statusCode == 200) {
        ...
        this.facilitatorService.createCourse(course).subscribe((courseData) => {
          console.log(courseData);
          if (courseData.statusCode == 200) {
            if (courseData.statusMessage == 'Course Already Exists') {
              this.toastr.success(courseData.statusMessage);
            } else {
              this.CourseId = courseData.data.id;
              this.authService.storeCourseId(this.CourseId);

              const video_data = this.videoFile;
              const data = new FormData();
                    ...
                    this.facilitatorService
                      ...
                      .subscribe((preview) => {
                        ...
                        if (preview.statusCode == 200) {
                          ...
                          this.coursePreview = preview.data;
                          ...
                          'No Course with the specified ID'
                        ) {
                          this.toastr.info(preview.statusMessage);
                        } else {
                          this.toastr.error(
                            preview.statusMessage,
                            'Something Went Wrong!',
            ...
            this.toastr.error(
              courseData.stausMessage,
              'Something Went Wrong!',
              {
                timeOut: 3000,
              }
            );
            this.error = courseData.statusMessage;
            this.success = null;
          }
        });
      } else {
  ...
  onSelectVideo(event) {
    ...
    var courseVideo = document.getElementById('videoImage') as HTMLImageElement;
    courseVideo.src = 'assets/img/video-placeholder.gif';
    // event.target.files[0]
    console.log(event.target.files[0]);
  }
    ...
    this.facilitatorService.createCourseTopics(course).subscribe(
      (data) => {
        ...
        if (data.statusMessage == 'Course Topics Created Successfully') {
          // var idArray = document.querySelectorAll("div[id='schkbox']");

          //   for (var i = 0; i < idArray.length; i++) {
          //     const input = idArray[i] as HTMLInputElement;
          //     input.checked = false;
          //   }
          var toggleLecture = document.getElementsByClassName(
            'lectureSection'
          ) as any;
          for (var i = 0; i < toggleLecture.length; i++) {
            const div = toggleLecture[i];
  ...
  remove(obj) {
    ...
    let indexx = this.objectiveContents.indexOf(obj);
    this.objectiveContents.splice(indexx, 1);
  }

  addObjective(obj) {
    this.objective = obj;
    this.objectiveContents.push(obj);
    (document.getElementById('courseObjective') as HTMLInputElement).value = '';
  }

  removeReq(req) {
    console.log('now');

    let indexx = this.objectiveContents.indexOf(req);
    this.objectiveContents.splice(indexx, 1);
  }

  addRequirement(req) {
    this.requirementContents.push(req);
    (document.getElementById('courseRequirement') as HTMLInputElement).value =
      '';
  }
  ...
  addVideo() {
    ...
    const file_data = this.videoFile;
    const data = new FormData();
    data.append('File', file_data);
    data.append('AppId', '1');
    data.append('FolderTypeId', '5');
    ...
    this.authService.uploadImage(data).subscribe((response) => {
      if (response.statusCode == 200) {
        ...
        if (response.fileData.fileUrl) {
          ...
          // this.courseTopicVid.Materials = this.courseTopicVideoContents;
          // this.courseTopicVid.CourseId = this.CourseId;

          this.courseTopicVideoContents.push(courseContent);
          console.log(this.courseTopicVideoContents);
          // (document.getElementById("courseRequirement") as HTMLInputElement).value = '';
        } else {
          this.toastr.error('An Error Occured, Please try again', 'Error', {
            timeOut: 3000,
        ...
        this.toastr.error(response.stausMessage, 'Something Went Wrong!', {
          timeOut: 3000,
        });
  ...
  removeVideo(vid) {
    ...
    let indexx = this.courseTopicVideoContents.indexOf(vid);
    this.courseTopicVideoContents.splice(indexx, 1);
  }

  createMultipleCourseTopicVideos() {
    console.log(this.courseTopicVideoContents);
    this.courseTopicVid.Materials = this.courseTopicVideoContents;
    this.courseTopicVid.CourseId = this.CourseId;
    (this.courseTopicVid.FacilitatorId = this.FacilitatorId),
      (this.courseTopicVid.CourseTopicId = this.courseTopicId),
      console.log(this.courseTopicVid);

    if (this.courseTopicVideoContents.length == 0) {
      this.toastr.error(
    ...
    this.facilitatorService
      .createMultipleCourseTopicVideos(this.courseTopicVid)
      .subscribe(
        (data) => {
          console.log(data);
  ...
  createMultipleCourseObjectives() {
    console.log(this.objectiveContents);
    this.objectiveString.Objective = this.objectiveContents;
    this.objectiveString.CourseId = this.CourseId;

    console.log(this.objectiveString);

    if (this.objectiveContents.length == 0) {
      this.toastr.error(
    ...
    this.facilitatorService
      .createMultipleCourseObjectives(this.objectiveString)
      .subscribe(
        (data) => {
          console.log(data);
          if (data.statusCode == 200) {
  ...
  createMultipleCourseRequirements() {
    console.log(this.requirementContents);
    this.requirementString.Requirement = this.requirementContents;
    this.requirementString.CourseId = this.CourseId;

    console.log(this.requirementString);

    if (this.requirementContents.length == 0) {
      this.toastr.error(
    ...
    this.facilitatorService
      .createMultipleCourseRequirements(this.requirementString)
      .subscribe(
        (data) => {
          console.log(data);
          if (data.statusCode == 200) {
  ...
  // onUpdateService() {
  //   this.loading = true;
  //   this.profileService.updateBusinessService(this.model2).subscribe((item) => {
  //     if (item.status == "Success") {
  //       this.loading = false;
  //       this.services = this.services.filter((ser) => {
  //         return ser.id !== this.model2.id;
  //       });

  //       this.services.splice(0, 0, this.model2);
  ...
  createCourseCourseTopicMaterial() {
    ...
    this.facilitatorService.createCourseTopicMaterial(courseContent).subscribe(
      (data) => {
        console.log(data);
        if (data.statusCode == 200) {
    ...
    this.authService.uploadImage(data).subscribe((response) => {
      if (response.statusCode == 200) {
        ...
        if (response.fileData.fileUrl) {
          ...
          this.facilitatorService
            .createCourseTopicVideo(courseContent)
            .subscribe(
              (data) => {
                console.log(data);
                if (data.statusCode == 200) {
