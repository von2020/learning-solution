...
@Component({
  selector: 'app-forgot-password-facilitator',
  templateUrl: './forgot-password-facilitator.component.html',
  styleUrls: ['./forgot-password-facilitator.component.css']
})
export class ForgotPasswordFacilitatorComponent implements OnInit {
  Email: any;
  Code: any;
  NewPassword: any;
  ConfirmPassword: any;
...
return false;
    ...
    this.authService.resetFacilitatorPassword(this.Email).subscribe(data => {
      console.log(data);
  ...
  onChangePasswordSubmit() {
    ...
    this.authService.changeFacilitatorPassword(JSON.stringify(user)).subscribe(data => {
      console.log(data);
      if(data.statusCode == 200){
        this.toastr.success(data.statusMessage);
        // alert(data.statusMessage)
